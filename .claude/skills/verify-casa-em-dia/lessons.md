...
## #14 — 29/06/2026 — Tabela `eventos_silenciados` referenciada no script mas nunca criada no banco
**O que quebrou:** nenhuma notificação chegava nos celulares — nem pergunta de gasto, nem contas vencendo, nada. O GitHub Actions mostrava execução bem-sucedida (sem erro de CI), mas na prática o script travava.
**Causa raiz:** `notificar.py` consulta a tabela `eventos_silenciados` logo no início da seção de eventos do calendário (linha 183), mas essa tabela nunca foi criada — não existe migration pra ela. O `r.raise_for_status()` dentro da função `rest()` lançava exceção HTTP (404 do PostgREST), derrubando o script inteiro **antes** de chegar na seção "pergunta diária". O GitHub Actions marcava o job como `failure`, mas quando rodado com `workflow_dispatch` sem checar o log detalhado, isso passava em branco.
**Correção:** criado `supabase/migration_eventos_silenciados.sql` e aplicado no banco via `scripts/rodar_migration.py`. Tabela `eventos_silenciados(id, event_id, user_id, created_at)` com RLS dono-only e FK em cascade pra `events` e `auth.users`.
**Prevenção (novo item no checklist abaixo, item 17):** sempre que adicionar consulta a uma tabela nova no `notificar.py` (ou em qualquer script que chame `rest()`), confirmar que a tabela existe no banco antes de commitar — ou que a migration foi aplicada. Grep por `rest(url, key, "` no script e comparar com as tabelas listadas em `supabase/migrations_aplicadas.txt` + as tabelas hardcoded conhecidas.

## #15 — 29/06/2026 — Só 1 pergunta diária "Teve gasto?" em vez de 2
**O que quebrou:** o usuário esperava 2 notificações por dia perguntando sobre gastos (uma no meio do dia, uma à noite). O código só tinha 1 (às 20h), com chave `pergunta_gasto:{hoje}`.
**Causa raiz:** nunca foi implementado o segundo horário — ficou como intenção não concretizada.
**Correção:** `notificar.py` agora tem dois turnos: 12h (chave `pergunta_gasto_manha:{hoje}`) e 20h (chave `pergunta_gasto_noite:{hoje}`). O body da mensagem é diferente pra cada turno. As chaves antigas `pergunta_gasto:{hoje}` já estão na tabela `notificacoes_enviadas` — como foram renomeadas, os registros antigos não conflitam e as duas novas disparam normalmente.
**Prevenção:** ao definir quantas vezes uma notificação dispara por dia, escrever isso explicitamente num comentário no código e testar localmente com `python scripts/notificar.py` antes de commitar.

## #16 — 30/06/2026 — Burst de notificações: várias chegando juntas são suprimidas pelo Android/MIUI
**O que quebrou:** quando o novo código de notificações de calendário foi deployado, 5 notificações informativas (agenda hoje, agenda amanhã, calendário Jéssica, resumo semana, pergunta gasto) ficaram pendentes e foram enviadas todas na mesma execução de 15 min. Jéssica recebeu só 1 das 5 no celular — as outras chegaram ao FCM (sucesso HTTP) mas nunca apareceram na tela.
**Causa raiz:** Android/MIUI (e Android em geral) tem comportamento documentado de suprimir ou colapsar notificações quando muitas chegam do mesmo app em burst (menos de 2 min de diferença). O sistema só exibe a primeira ou a última. Não há erro no servidor nem no código — o FCM confirma entrega, mas o OS filtra na ponta.
**Correção:** parâmetro `urgente=False` na função `enviar_pra_lista`. Cada sub recebe no máximo 1 notificação informativa por execução de 15min; as pendentes ficam para a próxima. Notificações urgentes (conta vencendo, evento chegando) usam `urgente=True` e passam sem limite.
**Prevenção (novo item no checklist, item #19):** sempre que adicionar um novo tipo de notificação informativa (não urgente), garantir que a chamada usa `urgente=False` (padrão). Nunca usar `urgente=True` para notificações que podem esperar 15 min. Regra geral: urgente = "algo acontece agora e não pode esperar"; informativo = "lembrete / pergunta / resumo".
