...
# Tabelas que o app consulta — se qualquer uma sumir, alguma tela quebra.
TABELAS_BANCO = [
    "bill_payments", "fixed_bills", "faturas_cartao", "cartoes", "events",
...
def main():
    ...
    # ── 1. Tabelas usadas pelo app ──────────────────────────────────────────
    print("1. Tabelas usadas pelo app:")
    for tabela in TABELAS_BANCO:
        status, resp = rest_get(url, key, tabela, {"select": "count", "limit": "1"})
        if status == 200:
            print(f"   [OK] {tabela}")
        elif status == 404:
            msg = f"Tabela '{tabela}' NÃO EXISTE no banco — o app vai quebrar ao carregar"
    ...
    secrets_necessarios = ["SUPABASE_URL", "SUPABASE_SERVICE_ROLE_KEY"]
...
"""
            criar_issue(gh_token, gh_repo, f"[Auto] Casa em Dia com erro — {hoje}", corpo)
        else:
